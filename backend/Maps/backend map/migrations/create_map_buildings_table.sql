...
CREATE TABLE IF NOT EXISTS map_buildings (
  ...
  description TEXT,
  svg_id TEXT NOT NULL,  -- Removed UNIQUE constraint (multiple buildings can share same SVG element)
  node_id INTEGER REFERENCES navigation_nodes(node_index) ON DELETE SET NULL,
  zone_id INTEGER,
...
CREATE OR REPLACE FUNCTION update_map_buildings_timestamp()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_update_map_buildings_timestamp
  BEFORE UPDATE ON map_buildings
  FOR EACH ROW
  EXECUTE FUNCTION update_map_buildings_timestamp();
